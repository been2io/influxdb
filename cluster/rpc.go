...
func (w *WriteShardResponse) SetCode(code int) { w.pb.Code = int32(code) }
...
func (r *ExecuteStatementRequest) SetDatabase(database string) { r.pb.Database = database }
...
// SetCode sets the Code
func (w *ExecuteStatementResponse) SetCode(code int) { w.pb.Code = int32(code) }
...
func (r *FieldDimensionsResponse) MarshalBinary() ([]byte, error) {
	...
	pb.Fields = make(map[string]int32, len(r.Fields))
...
func (r *FieldDimensionsResponse) UnmarshalBinary(data []byte) error {
	...
	for s, t := range pb.GetFields() {
		r.Fields[s] = influxql.DataType(t)
	}
