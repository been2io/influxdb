...
func NewQueryExecutor() *QueryExecutor {
	return &QueryExecutor{
		...
		statMap:   expvar.NewMap("queryExecutor"),
	}
}

// ExecuteQuery executes each statement within a query.
func (e *QueryExecutor) ExecuteQuery(q *influxql.Query, database string, chunkSize int, closing chan struct{}) <-chan *query.Result {
	results := make(chan *query.Result)
	go e.executeQuery(q, database, chunkSize, closing, results)
	return results
}

func (e *QueryExecutor) executeQuery(q *influxql.Query, database string, chunkSize int, closing chan struct{}, results chan *query.Result) {
	defer close(results)
	...
	for ; i < len(q.Statements); i++ {
		...
		if stmt, ok := stmt.(*influxql.SelectStatement); ok {
			if err := e.executeSelectStatement(stmt, chunkSize, i, results, closing); err != nil {
				results <- &query.Result{StatementID: i, Err: err}
				break
			}
			continue
		}
...
func (e *QueryExecutor) executeSelectStatement(stmt *influxql.SelectStatement, chunkSize, statementID int, results chan *query.Result, closing <-chan struct{}) error {
	...
	_, t, err := influxql.ConditionExpr(stmt.Condition, &influxql.NowValuer{Now: now})
