...
func (dst *SpecRequest) XXX_Merge(src proto.Message) {
	xxx_messageInfo_SpecRequest.Merge(dst, src)
}
...
func init() {
	proto.RegisterType((*SpecRequest)(nil), "influxdata.platform.storage.SpecRequest")
...
type StorageClient interface {
	...
	ExecSpec(ctx context.Context, in *SpecRequest, opts ...grpc.CallOption) (Storage_ExecSpecClient, error)
...
func (c *storageClient) ExecSpec(ctx context.Context, in *SpecRequest, opts ...grpc.CallOption) (Storage_ExecSpecClient, error) {
	stream, err := c.cc.NewStream(ctx, &_Storage_serviceDesc.Streams[2], "/influxdata.platform.storage.Storage/ExecSpec", opts...)
	if err != nil {
		return nil, err
	}
	x := &storageExecSpecClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type storageExecSpecClient struct {
	grpc.ClientStream
}
...
type StorageServer interface {
	...
	ExecSpec(*SpecRequest, Storage_ExecSpecServer) error
	// TagKeys performs a read operation for tag keys
...
func _Storage_ExecSpec_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(SpecRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(StorageServer).ExecSpec(m, &storageExecSpecServer{stream})
}
...
type storageExecSpecServer struct {
	grpc.ServerStream
}
...
var _Storage_serviceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "ExecSpec",
			Handler:       _Storage_ExecSpec_Handler,
			ServerStreams: true,
		},
		{
			StreamName:    "TagKeys",
...
func (m *SpecRequest) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.Request) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintStorageCommon(dAtA, i, uint64(len(m.Request)))
